...
In `asktheexperts/urls.py`, all the URL configuration of the app is defined, which means all of the url paths are there.
...
`asktheexperts/templates/asktheexperts/index.html` is the file for the index page that works like a landing page since it only has a *descriptive* message and two buttons: One that redirects to the page with all of the questions made on the website and the other redirects to the page where a user can ask a question.
